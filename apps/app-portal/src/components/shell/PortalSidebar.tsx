...
const devNavItems = [
  { href: "/app/devtools", label: "DevTools", icon: Settings },
];

export function PortalSidebar() {
  const pathname = usePathname();
  const showDevTools = process.env.NEXT_PUBLIC_DEMO_TOOLS === "true";

  return (
    <div className="w-64 border-r bg-muted/40 flex flex-col h-screen">
      <div className="p-4 space-y-2 flex-1">
        <h2 className="text-lg font-semibold mb-4 px-3">Portal</h2>
      ...
      <div className="p-4 border-t">
        <div className="text-xs text-muted-foreground">
          <div className="font-semibold">Relique Portal</div>
          <div className="mt-1">v0.1.0 (demo)</div>
        </div>
      </div>
    </div>
  );
